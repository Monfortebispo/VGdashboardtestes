...
assert(/navigationMetrics\s*:\s*\(\)\s*=>\s*modernViewRouter\.navigationMetrics\(\)/.test(main), 'entrada moderna deve expor métricas de navegação');
...
assert(/if\s*\(\s*!targetedRefresh\s*\)\s*await\s+this\.runtime\.refresh\?\.\(\)/.test(router), 'refreshAll deve permanecer apenas como fallback de vistas ainda legadas');
