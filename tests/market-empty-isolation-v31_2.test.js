...
assert(code.includes('function resetMarketDerivedUi()'),'V31.2 deve limpar leituras derivadas do mercado anterior');
...
assert(code.includes("if(hm&&!hasPnl)hm.textContent='—'"),'período do topo não pode herdar o mês do mercado anterior');
...
assert(classes.has('vg-market-no-pnl')&&classes.has('vg-market-empty'),'mercado vazio deve ativar isolamento visual');
assert.strictEqual(el('headerHotels').textContent,'0');
assert.strictEqual(el('headerMes').textContent,'—');
assert(els.emptyH2.textContent.includes('Brasil'),'empty state deve identificar o mercado Brasil');
assert(els.emptyP.innerHTML.includes('não é usado nesta análise'),'mensagem deve garantir separação do mercado anterior');

console.log('✓ V31.2: mercado vazio não herda KPIs, Central, Ficha ou contexto de PT+ES');
