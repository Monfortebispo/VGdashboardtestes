...
assert(lazy.includes('assets/js/modules/audit-governance.js')&&lazy.includes('assets/css/audit-governance.css'),'os recursos de Auditoria & Governação v16 devem estar ligados ao carregador modular');
...
assert(lazy.includes('assets/js/modules/backup-recovery.js')&&lazy.includes('assets/css/backup-recovery.css'),'os recursos de Backup & Recuperação v17 devem estar ligados ao carregador modular');
...
assert(lazy.includes('assets/js/modules/anomaly-detection.js'),'o módulo de anomalias v13 deve estar ligado ao carregador modular');
assert(lazy.includes('assets/css/anomaly-detection.css'),'o CSS de anomalias v13 deve estar ligado ao carregador modular');
assert(html.includes('id="view-datacenter"')&&html.includes('id="dcRoot"'),'a V10 deve incluir o Centro de Dados');
assert(lazy.includes('assets/js/modules/data-center.js'),'o módulo do Centro de Dados deve estar ligado ao carregador modular');
assert(lazy.includes('assets/css/data-center.css'),'o CSS do Centro de Dados deve estar ligado ao carregador modular');
...
assert(lazy.includes('assets/js/modules/benchmarking.js'),'o módulo de Benchmarking v11 deve estar ligado ao carregador modular');
assert(lazy.includes('assets/css/benchmarking.css'),'o CSS de Benchmarking v11 deve estar ligado ao carregador modular');
...
assert(lazy.includes('assets/js/modules/forecast-scenarios.js'),'o módulo Forecast & Cenários v12 deve estar ligado ao carregador modular');
assert(lazy.includes('assets/css/forecast-scenarios.css'),'o CSS Forecast & Cenários v12 deve estar ligado ao carregador modular');
assert(html.includes('id="nav-forecast"'),'Forecast & Cenários deve estar acessível pela navegação');
const riPos=lazy.indexOf('assets/js/modules/revenue-intelligence.js');
const forecastPos=lazy.indexOf('assets/js/modules/forecast-scenarios.js');
