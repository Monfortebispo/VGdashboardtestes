...
const pkg=require('../package.json'),html=read('index.html'),lazy=read('assets/js/core/08-performance-loader-v35_7.js'),js=read('assets/js/modules/operations-domains-v33.js'),css=read('assets/css/operations-domains-v33.css'),nav=read('assets/js/ui/vg-operations-2-v30.js'),search=read('assets/js/ui/global-search.js'),pdf=read('assets/js/modules/pdf-export.js'),ig=read('assets/js/modules/instagram.js'),server=read('netlify/functions/dashboard-sessao.js'),sw=read('service-worker.js'),seed=require('../assets/data/operations-seed-v33.json');
...
assert(lazy.includes('operations-domains-v33.js')&&lazy.includes('operations-domains-v33.css'),'carregador modular deve conhecer módulo/CSS V33');
...
assert(server.includes('ops-reputation-semester-')&&server.includes('ops-ab-')&&server.includes('ops-housekeeping-')&&server.includes('buildVersion:"35.7"'),'backup/auditoria deve conhecer recursos V33');
...
assert(sw.includes("const CACHE_NAME = 'vg-operations-shell-v35-7'")&&!sw.includes('/assets/js/modules/operations-domains-v33.js')&&!sw.includes('/assets/js/modules/compras-ab-native-v35.js')&&!sw.includes('/assets/js/modules/housekeeping-native-v35.js'),'PWA V35.7 deve deixar domínios operacionais fora do pre-cache inicial');
...
assert(html.includes('V35.7 · Performance')&&nav.includes('Novos módulos já disponíveis'),'V35.7 deve ser imediatamente identificável após deploy');
console.log('✓ V35.7: integração nativa e retrocompatibilidade V33 auditadas');
