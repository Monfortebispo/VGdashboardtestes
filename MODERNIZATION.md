...
Portefólio, Reputação, Revenue e Custos continuam como adaptadores de compatibilidade. Ocupação já avançou para um controlador próprio e deixou de chamar diretamente `setView()`/`refreshAll()` no seu módulo moderno.

Ocupação é o primeiro domínio com corte real de dependência de dados e navegação:
- `occupancy-modern-bridge-v40.js` fornece acesso somente de leitura a `OCC_SNAPSHOTS` sem alterar `ocupacao.js`;
- a bridge sincroniza hotel e snapshot com os controlos legados, mas não controla a navegação;
...
- `occupancy-state.ts` mantém a seleção fora do DOM;
- `occupancy-controller.ts` coordena estado, dados e atualização seletiva;
- `modules/occupancy.ts` já não chama `legacyView`, `setView` nem `refreshAll`;
- toda esta camada permanece desligada do `index.html`, portanto não afeta a aplicação atual.
...
O `refreshAll()` legado continua disponível no adaptador do router para módulos que ainda não foram isolados. Para Ocupação, o runtime moderno já tenta atualização seletiva e usa o refresh global apenas como fallback de compatibilidade.

Próxima etapa da Fase 4:
1. completar a renderização de leitura da Ocupação através do controlador moderno;
...
- `tests/modern-occupancy-source.test.js` valida estado, controlador, bridge, fonte seletiva e ausência de navegação/refresh global no módulo moderno de Ocupação.
