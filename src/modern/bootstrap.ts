...
export async function openModernModule(id:ModernModuleId):Promise<void>{const mod=await loadModule(id);await mod.mount?.(document.body);}
export function resetModernDataCache():void{invalidateData();}
export const modernArchitecture=Object.freeze({status:'isolated',version:11,lazyModules:['portfolio','occupancy','reputation','revenue','financial-revenue','costs','pl-usali','approvals','city-ledger','purchases'] as const,navigationGroups:navigationGroups().map(group=>({name:group.name,views:group.items.map(item=>item.id)})),dataPlan:viewDataPlan(),dataStats:dataRegistryStats,navigationMetrics:()=>modernViewRouter.navigationMetrics()});
...
previewWindow.VG.modernPreview={router:modernViewRouter,navigation:modernNavigation,architecture:modernArchitecture,resetDataCache:resetModernDataCache};
