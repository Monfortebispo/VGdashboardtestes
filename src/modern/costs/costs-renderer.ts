...
function money(v:number):string{return new Intl.NumberFormat('pt-PT',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(v);}
...
export function renderCostsReadOnly(root:HTMLElement,selection:Readonly<CostsSelection>,actions:CostsRenderActions={}):HTMLElement{
  const source=currentCostsData();let host=root.querySelector<HTMLElement>('[data-modern-costs-readonly]');
  ...
  const hotels=[...new Set(source.records.map(r=>r.hotel))].sort((a,b)=>a.localeCompare(b,'pt'));
  const categories=[...new Set(source.records.map(r=>r.category))].sort((a,b)=>a.localeCompare(b,'pt'));
  const periods=[...new Set(source.records.map(r=>r.period))].sort((a,b)=>a.localeCompare(b,'pt'));
  let rows=source.records.filter(r=>(selection.hotel==='__all__'||r.hotel===selection.hotel)&&(selection.category==='__all__'||r.category===selection.category)&&(selection.period==='__all__'||r.period===selection.period));
  const controls=document.createElement('div');controls.dataset.modernCostsControls='true';
  const h=selectControl('Hotel',selection.hotel,[{value:'__all__',label:'Todos os hotéis'},...hotels.map(v=>({value:v,label:v}))]);
  ...
  const total=rows.reduce((s,r)=>s+r.value,0);const summary=document.createElement('p');summary.textContent=`${rows.length} registos · ${money(total)}`;
  ...
  const body=document.createElement('tbody');rows.slice(0,1000).forEach(r=>{const tr=document.createElement('tr');[r.hotel,r.category,r.period,money(r.value)].forEach(v=>{const td=document.createElement('td');td.textContent=v;tr.appendChild(td);});body.appendChild(tr);});table.appendChild(body);
