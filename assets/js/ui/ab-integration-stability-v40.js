(function(){
  ...
  function root(){
    try{return moduleRef()?.getRoot?.()||window.AB35Root||null;}
    catch(e){return null;}
  }
  ...
  function ensureStyle(r){
    ...
    style.id='vgAbEmbeddedStyleStable';
    style.textContent=`
      ...
      .view,.panel,.cards,.grid2,.tbl-wrap{max-width:100%!important}
  ...
  function ensureEmbeddedNav(r){
    ...
    const scope=r.querySelector('.ab35-scopebar');
    if(!oldNav||!scope)return;

    let wrap=r.getElementById&&r.getElementById('vgAbEmbeddedNav');
    let sel=r.getElementById&&r.getElementById('vgAbEmbeddedSelect');
    if(!wrap){
      wrap=document.createElement('div');
      wrap.id='vgAbEmbeddedNav';
      wrap.className='vg-ab-embedded-nav';
      const lab=document.createElement('label');
      lab.textContent='Análise';
      sel=document.createElement('select');
      sel.id='vgAbEmbeddedSelect';
      Object.keys(labels).forEach(function(v){
        const btn=oldNav.querySelector('.nav-btn[data-view="'+v+'"]');
        if(!btn)return;
        const opt=document.createElement('option');
        opt.value=v;opt.textContent=labels[v];sel.appendChild(opt);
      });
    ...
    const active=oldNav.querySelector('.nav-btn.on[data-view]');
    if(sel&&active&&labels[active.dataset.view])sel.value=active.dataset.view;
  }
  ...
  function integrate(){
    ...
    try{
      ...
      console.warn('[VG A&B stability] integração',err);
      return false;
    }
  }
  ...
  function watchRoot(r){
    ...
    rootObserver=new MutationObserver(function(){
      ...
      retryTimer=setTimeout(integrate,20);
    });
    try{rootObserver.observe(r,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});}catch(e){}
  }

  function retryBurst(){
    [0,30,80,160,320,650,1200,2200,4000,7000].forEach(function(ms){setTimeout(function(){
      if(!integrate()&&window.VG?.modernPreview?.repairAB&&ms>=320){
        try{void window.VG.modernPreview.repairAB().then(integrate);}catch(e){}
      }
    },ms);});
  }

  function installDocObserver(){
    if(docObserver)return;
    docObserver=new MutationObserver(function(mutations){
      for(const m of mutations){
        if(m.type==='childList'&&(document.getElementById('ab35NativeMount')||inAB())){retryBurst();break;}
      }
    });
  ...
  document.addEventListener('click',function(e){
    const nav=e.target&&e.target.closest&&e.target.closest('#nav-ab');
    if(nav)retryBurst();
  },false);
  window.addEventListener('hashchange',function(){if(inAB())retryBurst();});
  window.addEventListener('vg-modern-preview-ready',retryBurst);
  installDocObserver();
  retryBurst();
})();
