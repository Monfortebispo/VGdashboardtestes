...
(function(){
  ...
  function buildHotelsAndKpis(arr){
    ...
    add(arr,{type:'data',title:'Receita Detalhada',subtitle:'PdV, família, subfamília, grupo e artigo',keywords:'receita detalhada vendas artigos ponto venda pvd mix',open:()=>window.setView?.('receitasdet')});
    add(arr,{type:'article',title:'Compras & A&B',subtitle:'Food Cost, Beverage Cost, stock, receituário e inteligência',keywords:'compras ab food cost beverage stock fichas tecnicas receitas cocktails',open:()=>window.setView?.('ab')});
    add(arr,{type:'data',title:'Housekeeping · Inventário Têxtil',subtitle:'Stock, quebras, campanhas e necessidades de compra',keywords:'housekeeping inventario textil roupas quebras turcos lencois par stock',open:()=>window.setView?.('housekeeping')});
    add(arr,{type:'data',title:'Reputação & Guest Experience',subtitle:'Visão executiva, semanal, semestral e hotel',keywords:'reputacao reviewpro semanal semestral gri cqi reviews',open:()=>window.setView?.('reputacao')});
    try{const lib=window.VG?.domains33?.state?.seed?.technicalLibrary||{};for(const x of [...(lib.recipes||[]),...(lib.products||[])].slice(0,260)){add(arr,{type:'article',title:`Ficha Técnica · ${x.name}`,subtitle:[x.category,x.brandScope,x.version].filter(Boolean).join(' · '),keywords:[x.collection,x.source,(x.ingredients||[]).map(i=>i.ingredient).join(' ')].filter(Boolean).join(' '),open:()=>{window.setView?.('ab');setTimeout(()=>{const root=document.getElementById('abHubRoot');if(root){root.dataset.tab='recipes';window.VG?.domains33?.renderAB?.();const q=document.getElementById('ft33Search');if(q){q.value=x.name;q.dispatchEvent(new Event('input',{bubbles:true}));}}},80);}});}}catch(e){}
