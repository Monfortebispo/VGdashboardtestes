...
(function(){
  ...
  function buildModel(){
    ...
    [...data.priorities,...operational,...riPriorities].forEach(p=>{
    ...
    const priorities=[...merged.values()].map(p=>{
      ...
      if(kinds.length>1){
        p.kind='mixed';
        p.title='Prioridade combinada — '+kinds.map(k=>k==='revenue'?'Revenue':k==='data'?'Dados':'Operação').join(' + ');
    ...
    const opportunities=[...riOpp,...kpiOpp].sort((a,b)=>b.score-a.score).filter((row,idx,arr)=>arr.findIndex(x=>x.hotel===row.hotel&&x.title===row.title)===idx).slice(0,6);
    ...
    return {
      hotels, operational, data, ri, priorities, opportunities, actionStats, actionWatch,
  ...
  function priorityHtml(p,i){
    ...
    const type=p.kind==='data'?'Dados':p.kind==='revenue'?'Revenue':p.kind==='mixed'?'Decisão integrada':'Operação';
    const details=(p.reasons||[]).slice(0,3).join(' · ');
    const actionLabel=p.kind==='revenue'?'Abrir Revenue Intelligence':'Abrir Ficha do Hotel';
    const action=p.kind==='revenue'?`opsGoTo('revenueint')`:`opsOpenHotel(${JSON.stringify(p.hotel)})`;
  ...
  function render(){
    ...
    if(stats) stats.innerHTML=[
      ...
      statHtml('Qualidade dados',String(m.data.all.length),m.data.all.length?'ocorrências a validar':'sem incoerências estruturais','health')
    ].join('');
