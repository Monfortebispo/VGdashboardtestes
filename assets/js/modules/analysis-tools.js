...
// ==========================================================
const ALERT_RULES = [
  { id:'gop_neg',    label:'GOP com sede negativo',      severity:'red',    check:(h)=>{ const g=gop(h,YR_CUR); return g!=null&&g<0; } },
  { id:'gop_low',    label:'Margem GOP com sede < 20%',  severity:'red',    check:(h)=>{ const p=gopPct(h,YR_CUR); return p!=null&&p<20; } },
  { id:'occ_low',    label:'Occupancy < 40%',            severity:'orange', check:(h)=>{ const o=occ(h,YR_CUR); return o!==null&&o<40; } },
  { id:'occ_drop',   label:'Occupancy caiu > 10pp',      severity:'orange', check:(h)=>{ const o25=occ(h,YR_PREV),o26=occ(h,YR_CUR); return o25!==null&&o26!==null&&(o26-o25)<-10; } },
  { id:'labour_hi',  label:'Pessoal > 40% Receita',      severity:'red',    check:(h)=>{ const r=n(RAW.hotels_ops[h]?.['Receita Total']?.[YR_CUR]), p=n(RAW.hotels_costs[h]?.PESSOAL?.[YR_CUR]); return r>0&&p/r*100>40; } },
  { id:'energy_hi',  label:'Energia > 8% Receita',       severity:'orange', check:(h)=>{ const r=n(RAW.hotels_ops[h]?.['Receita Total']?.[YR_CUR]), e=n(RAW.hotels_costs[h]?.ENERGIA?.[YR_CUR]); return r>0&&e/r*100>8; } },
  { id:'maint_hi',   label:'Manutenção > 8% Receita',    severity:'orange', check:(h)=>{ const r=n(RAW.hotels_ops[h]?.['Receita Total']?.[YR_CUR]), m=n(RAW.hotels_costs[h]?.MANUTENÇÃO?.[YR_CUR]); return r>0&&m/r*100>8; } },
  { id:'rev_drop',   label:'Receita caiu > 10% vs ano anterior', severity:'red',    check:(h)=>{ const r25=n(RAW.hotels_ops[h]?.['Receita Total']?.[YR_PREV]),r26=n(RAW.hotels_ops[h]?.['Receita Total']?.[YR_CUR]); return r25>0&&(r26-r25)/r25*100<-10; } },
  { id:'adr_drop',   label:'ADR caiu > 5% vs ano anterior',      severity:'orange', check:(h)=>{ const a25=adr(h,YR_PREV),a26=adr(h,YR_CUR); return a25&&a26&&(a26-a25)/a25*100<-5; } },
];
...
function alertasRender() {
  ...
  document.getElementById('alertas-grid').innerHTML = renderDataQualityBlock(dataIssues) + hotelAlerts.map(({h, triggered}) => {
    ...
    const badges = triggered.map(r => `<span style="background:${sevBg[r.severity]};border:1px solid ${sevColor[r.severity]};color:${sevColor[r.severity]};font-size:9px;font-weight:700;padding:3px 8px;border-radius:6px">${r.label}</span>`).join('');
...
function waBuildAlertas() {
  ...
  if (!hotelAlerts.length) { lines.push('🟢 Nenhum alerta activo.'); }
    ...
    hotelAlerts.forEach(({h, triggered}) => {
      ...
      triggered.forEach(r => lines.push(`　${r.severity==='red'?'🔴':'🟡'} ${r.label}`));
    });
  }
