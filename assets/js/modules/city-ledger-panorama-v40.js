// VG Operations — City Ledger: Panorama por Hotel V41
(function(){
'use strict';
if(window.__VG_CITY_LEDGER_PANORAMA_V41__)return;
window.__VG_CITY_LEDGER_PANORAMA_V41__=true;
...
function selectedHotel(){
  ...
  return s.panoramaHotel&&hotels.includes(s.panoramaHotel)?s.panoramaHotel:'';
}
function setSelectedHotel(h){const s=state();if(s)s.panoramaHotel=h||'';}

function styles(){
  if(document.getElementById('clPanoramaV41Style'))return;
  const st=document.createElement('style');st.id='clPanoramaV41Style';st.textContent=`
  .cl-panorama-v41{min-height:0;position:relative}.cl-panorama-tools{display:flex;align-items:end;gap:12px;flex-wrap:wrap;margin-bottom:12px}.cl-panorama-tools label{display:grid;gap:5px;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted,#8ca3bd)}.cl-panorama-tools select{min-width:260px;background:#0b2036;color:#fff;border:1px solid #294865;border-radius:8px;padding:9px 11px;pointer-events:auto;cursor:pointer;position:relative;z-index:4}
...
function panoramaHtml(){
  const hotels=hotelData();if(!hotels.length)return'<div class="cl-panorama-v41"><div class="cl-empty">Sem saldos em dívida na moeda principal para construir o panorama.</div></div>';
  const sel=selectedHotel(),h=hotels.find(x=>x.hotel===sel);
  return `<div class="cl-panorama-v41"><div class="cl-panorama-tools"><label>Hotel<select id="clPanoramaHotel"><option value="">Todos os hotéis</option>${hotels.map(x=>`<option value="${esc(x.hotel)}" ${x.hotel===sel?'selected':''}>${esc(x.hotel)}</option>`).join('')}</select></label>${sel?'<button class="cl-secondary" data-cl-panorama-all>← Voltar ao panorama geral</button>':''}</div>${h?hotelDetailHtml(h):matrixHtml(hotels)}</div>`;
}
function activateTabUI(root){
  ...
  if(!btn){
    btn=document.createElement('button');btn.dataset.clTab='hotelpanorama';btn.textContent='Panorama por Hotel';
    const tabs=root.querySelector('.cl-tabs'),after=tabs?.querySelector('[data-cl-tab="hotels"]');
    if(tabs){after?.after(btn);if(!after)tabs.appendChild(btn);}
  }
  btn.classList.add('active');
}
function bindPanorama(body,s){
  body.querySelector('#clPanoramaHotel')?.addEventListener('change',e=>{setSelectedHotel(e.target.value);renderPanorama(true);});
  body.querySelector('[data-cl-panorama-all]')?.addEventListener('click',()=>{setSelectedHotel('');renderPanorama(true);});
  body.querySelectorAll('[data-cl-panorama-hotel]').forEach(tr=>tr.addEventListener('click',()=>{setSelectedHotel(tr.dataset.clPanoramaHotel);renderPanorama(true);}));
...
function renderPanorama(force=false){
  ...
  if(!force&&body.querySelector('.cl-panorama-v41'))return true;
  const y=window.scrollY;
  body.innerHTML=panoramaHtml();bindPanorama(body,s);
  requestAnimationFrame(()=>window.scrollTo({top:y,left:window.scrollX,behavior:'auto'}));
  return true;
}
function ensureTab(){
  const root=document.getElementById('cityLedgerRoot'),a=api(),s=state();if(!root||!a||!s)return;
  let btn=root.querySelector('[data-cl-tab="hotelpanorama"]');
  if(!btn){
    btn=document.createElement('button');btn.dataset.clTab='hotelpanorama';btn.textContent='Panorama por Hotel';
    const tabs=root.querySelector('.cl-tabs'),after=tabs?.querySelector('[data-cl-tab="hotels"]');
  ...
  if(btn&&!btn.dataset.panoramaBound){
    btn.dataset.panoramaBound='1';
    btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();s.tab='hotelpanorama';renderPanorama(true);});
  }
  if(s.tab==='hotelpanorama'){
    const body=root.querySelector('.cl-body');
    if(body&&!body.querySelector('.cl-panorama-v41'))renderPanorama(true);
    else activateTabUI(root);
  }
}
function init(){
  styles();ensureTab();
  timer=setInterval(ensureTab,700);
  document.addEventListener('click',e=>{if(e.target.closest?.('#nav-cityledger'))setTimeout(ensureTab,120);},false);
  window.VG?.events?.on?.('cityledger:changed',()=>setTimeout(()=>renderPanorama(true),80));
  window.VG?.events?.on?.('cityledger:diligence',()=>setTimeout(()=>renderPanorama(true),80));
