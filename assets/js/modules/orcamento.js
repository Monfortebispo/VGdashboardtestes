...
const ORC_REVENUE_FACTOR = 1.05;              // Receitas = 2025 +5%
const ORC_COST_FACTOR = 1.08;                 // Custos = 2025 +8%
function orcIsFixedBudgetMonth(m){ return Number(m) >= ORC_FIXED_START_MONTH; }
function orcHasRealData2026(hotel, mes){
...
function orcUltimoMesReal2026(hotel){
  const meses = Object.keys(STORE || {}).map(Number).filter(m => orcHasRealData2026(hotel, m)).sort((a,b)=>a-b);
  return meses.length ? meses[meses.length-1] : 0;
}

function orcInjectForecastToStore() {
  // Determinar meses já realizados com dados reais de 2026
  const yr = String(YR_CUR), yrp = String(YR_PREV);
  const mesesComDados2026 = Object.keys(STORE).map(Number).filter(m => {
    const store = STORE[m];
    if (!store || store[ORC_FORECAST_TAG]) return false;
    // Tem dados reais de 2026 se pelo menos um hotel tem receita > 0
    return (store.hotel_list || []).some(h => {
      const ops = store.hotels_ops?.[h];
      return ops && n(ops['Receita Total']?.[yr]) > 0;
    });
  });
  const ultimoMesReal = mesesComDados2026.length > 0 ? Math.max(...mesesComDados2026) : 0;

  // Para cada mês seleccionado que não tem dados reais de 2026, gerar previsão
  for (const mes of selectedMeses) {
    // Já tem dados reais de 2026 — não sobrescrever
    if (STORE[mes] && !STORE[mes][ORC_FORECAST_TAG]) {
      const hasReal2026 = (STORE[mes].hotel_list || []).some(h => {
        const ops = STORE[mes].hotels_ops?.[h];
        return ops && n(ops['Receita Total']?.[yr]) > 0;
      });
      if (hasReal2026) continue;
    }

    // Verificar se há dados de 2025 para este mês (necessários para a previsão)
    if (!STORE[mes] || !STORE[mes].hotel_list) continue;
    const hasData2025 = (STORE[mes].hotel_list || []).some(h => {
      const ops = STORE[mes].hotels_ops?.[h];
      return ops && n(ops['Receita Total']?.[yrp]) > 0;
    });
    if (!hasData2025) continue;
    ...
    STORE[mes] = {
      ...STORE[mes],  // manter dados de 2025
...
function orcCalcSimple(hotel, mesPrev, yr, yrp, ultimoMesReal) {
  ...
  if (orcIsFixedBudgetMonth(mesPrev)) {
    // Regra definida: Julho em diante = 2025 como base +5% receitas e +8% custos
...
function orcCalc(hotel,mesPrev){
  ...
  // Um mês é "real" apenas se tiver dados reais de 2026; previsões sintéticas nunca contam como real
  const hasData2026 = orcHasRealData2026(hotel, mesPrev);
  ...
  if(hasData2026){
    ...
    return{real:true,mes:mesPrev,
      ...
      gop26:ops&&costs?n(ops['Receita Total']?.[yr])-n(costs.TOTAIS?.[yr]):null,
      gop25:ops&&costs?n(ops['Receita Total']?.[yrp])-n(costs.TOTAIS?.[yrp]):null,
      adr26:adrOficial(hotel,yr,d),adr25:adrOficial(hotel,yrp,d),
      quartos:occQuartosHotel(hotel),dias};
  }

  // Regra fixa: Julho a Dezembro usam 2025 como base (+5% receitas, +8% custos)
  if (orcIsFixedBudgetMonth(mesPrev)) {
  ...
  // Sazonalidade 2025 — usa TODOS os meses do STORE que têm dados de 2025
  // (inclui meses futuros de 2026 que só têm histórico 2025)
  ...
  for(let m=1;m<=12;m++){
    ...
    // Usa dados de 2025 (existem mesmo em meses ainda sem 2026)
    const ra=n(ops['Receita Alojamento']?.[yrp]),rfb=n(ops['Receita FB']?.[yrp]);
    if(ra===0&&rfb===0)continue; // sem dados de 2025 para este mês
  ...
  if(pu==null&&typeof OCC_SNAPSHOTS!=='undefined'&&OCC_SNAPSHOTS.length>0){
    ...
    // Usar ocupação do mesmo mês de 2025 como proxy base, ajustada pela tendência OTB
...
function orcRender(){
  ...
  const ultimoMes=orcUltimoMesReal2026(null)||5;
  ...
  <div style="margin-bottom:18px">
    ...
    <p style="color:var(--text-3);font-size:12px">Orçamento por hotel: meses reais mantidos; de julho a dezembro usa 2025 como base, com +5% nas receitas e +8% nos custos.</p>
  </div>
...
function orcRenderResult(){
  ...
  // Verificar se há dados de 2025 para o mês
  ...
  if(!temDados25 && !d25){
    el.innerHTML=`<div style="background:var(--surface-1);border:1px solid var(--border);border-radius:12px;padding:24px;text-align:center">
      ...
      <div style="font-size:14px;font-weight:700;color:var(--text-1);margin-bottom:8px">Dados de ${ORC_MESES_NOMES[ORC_STATE.mesPrev]} 2025 não carregados</div>
      <div style="font-size:12px;color:var(--text-3)">Para prever ${ORC_MESES_NOMES[ORC_STATE.mesPrev]} 2026, carrega o ficheiro P&L de ${ORC_MESES_NOMES[ORC_STATE.mesPrev]} 2025 em <b>Carregar Docs → Carregar Excel P&L</b>.</div>
    </div>`;
    return;
  }
  const d=orcCalc(ORC_STATE.hotel,ORC_STATE.mesPrev);
  const isReal=d.real;
  const badge=isReal?'<span style="background:#166534;color:#bbf7d0;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700">✓ Dados reais</span>':(d.fixedBudget?'<span style="background:rgba(251,191,36,.15);color:var(--gold);padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700">▶ Orçamento fixo 2025 +5/+8</span>':`<span style="background:rgba(251,191,36,.15);color:var(--gold);padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700">▶ Previsão ${d.cenario}</span>`);
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(155px,1fr));gap:10px;margin-bottom:20px">
    ${kpis.map(([l,v,v25])=>`<div style="background:var(--surface-2);border-radius:8px;padding:12px 14px;border-left:3px solid var(--gold)"><div style="font-size:10px;color:var(--text-3);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">${l}</div><div style="font-size:18px;font-weight:700;color:var(--gold)">${v}</div><div style="font-size:11px;color:var(--text-3);margin-top:2px">2025: ${v25}</div></div>`).join('')}
  </div>
  <table style="width:100%;border-collapse:collapse">
  <thead><tr style="background:var(--surface-3)"><th style="text-align:left;padding:9px 12px;font-size:11px;color:var(--text-3);font-weight:600;text-transform:uppercase;letter-spacing:1px">Rubrica</th><th style="text-align:right;padding:9px 12px;font-size:11px;color:var(--text-3);font-weight:600">2025 Real</th><th style="text-align:right;padding:9px 12px;font-size:11px;color:var(--gold);font-weight:600">${isReal?'2026 Real':'2026 Previsto'}</th><th style="text-align:right;padding:9px 12px;font-size:11px;color:var(--text-3);font-weight:600">Variação</th></tr></thead>
  <tbody>
  ...
  ${!isReal?`<tr><td colspan="4" style="padding:8px 12px;font-size:11px;color:var(--text-3);border-top:1px solid var(--border)">${d.fixedBudget?'ℹ Regra aplicada: receitas 2025 × 1,05 | custos 2025 × 1,08. Cenário e pickup não alteram julho-dezembro.':'ℹ Tendência YoY: Receitas ×'+fmt(d.factorAloj,2)+' | Custos ×'+fmt(d.factorCust,2)+' | Sazonalidade mês: '+fmt(d.pesoAloj*100,1)+'% da receita anual'+(d.pickupOcc!=null?' | Pickup: '+fmt(d.pickupOcc,1)+'%':'')}</td></tr>`:''}
  </tbody></table></div>`;
}
...
function orcExport(){
  ...
  const realizados=Object.keys(STORE).map(Number);
  const ultimoMes=Math.max(0,...realizados);
  const headers=['Mês','Tipo','Occ 2025 %','Rec Aloj 2025','Rec Aloj Prev','Rec FB 2025','Rec FB Prev','Rec Total 2025','Rec Total Prev','Custos 2025','Custos Prev','Pessoal Prev','Energia Prev','Comidas Prev','Bebidas Prev','Manut Prev','Operac Prev','GOP 2025','GOP Prev','Var Rec %','Var GOP %'];
  const rows=[headers];
  for(let m=1;m<=12;m++){
    const d=orcCalc(hotel,m);
    const tipo=d.real?'Real':(d.fixedBudget?'Orçamento fixo 2025 +5/+8':'Previsto');
