...
function apply(force=false){
  if(applying)return;const root=document.getElementById('abHubRoot');if(!root||!isTheoretical(root))return;
  const hs=hotels();let selected=currentSelected();if(selected!=='Todos'&&!hs.includes(selected)){selected='Todos';setSelected(selected);}
  const base=baseData();const sig=[selected,hs.join('|'),base.matched?.length||0,base.unmatched?.length||0].join('::');
  if(!force&&sig===lastSignature&&root.querySelector('#vgTheoryHotelToolbar'))return;
  applying=true;
  try{
    ...
    const select=toolbar.querySelector('#vgTheoryHotel');
    if(select)select.onchange=e=>{setSelected(e.target.value);lastSignature='';apply(true);};
...
function schedule(force=false){[0,120,400,900,1800,3200].forEach(ms=>setTimeout(()=>apply(force),ms));}
function init(){
  document.addEventListener('click',e=>{if(e.target?.closest?.('[data-abtab="theoretical"]')){lastSignature='';schedule(true);}},true);
  window.VG?.events?.on?.('revenue-detail:changed',()=>{lastSignature='';schedule(true);});
  window.VG?.events?.on?.('state:changed',()=>{if(isTheoretical(document.getElementById('abHubRoot'))){lastSignature='';schedule(true);}});
  schedule(true);
}
