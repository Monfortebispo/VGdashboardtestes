...
(function(){
  ...
  function selectTab(tab){const allowed=new Set(['overview','finance','revenue','efficiency','operation','reputation','actions','documents']);state.tab=allowed.has(tab)?tab:'overview';render();const dep=state.tab==='efficiency'?'unitEconomics':state.tab==='documents'?'documents':'';if(dep&&window.VG?.lazy?.ensureFeature){window.VG.lazy.ensureFeature(dep).then(()=>{try{if(typeof currentView==='undefined'||currentView==='hotel360')render();}catch(e){render();}}).catch(()=>{});}}
  function render(){
