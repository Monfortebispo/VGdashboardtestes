...
function riTargetOcc(h,m){
  // Objetivo dinâmico: OCC do ano anterior + 2 p.p. por hotel/mês.
  // Se não houver uma base real do ano anterior, devolve null: não cria um
  // objetivo artificial a partir da ocupação atual.
...
function riAskDashboard(){
  ...
  lines.push(`Forecast: ${riPct(forecast)} | Objetivo (${riPrevYear()} + 2 p.p.): ${riPct(target)} | Gap: ${forecast!=null&&target!=null?riFmt(target-forecast,1)+' pp':'—'}`);
